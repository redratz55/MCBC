# -*- coding: utf-8 -*-
"""
Created on Fri Apr  1 08:27:07 2022
...
names = ["t"+str(i) for i in range(20)]
#Load first dataset
df = pd.read_csv("segmentation.Test", index_col=False, skiprows = 1, names = names)
...
df2 = pd.read_csv("segmentation.data", index_col=False, skiprows = 1, names = names)
...
#Some weird NA value occurs for DF1 at index 0.
df.dropna(inplace = True)

#Seeded Train Test Split
x_train, x_test, y_train, y_test = train_test_split(df.drop('t0',  axis = 1), df['t0'], random_state = 2)




#Algorithm with restricting uper and lower bounds
...
print(f'Average Train Score: {np.mean(mcm_train_score)}, Train Deviation: {np.std(mcm_train_score)}')
print(f'\nBayes AVG Score: {np.mean(bayes_scores)}, Deviation: {np.std(bayes_scores)}, AVG Time: {np.mean(bayes_time)}')
